const SongModel = require("../model/song_model")
...
module.exports.getById = function (req, res) {
    ...
    SongModel.findById({ _id: id }).populate('playlist').exec(function (err, data) {
        if (err) {
...
module.exports.getAllSongs = function (req, res) {
    //REST
    SongModel.find().populate('playlist').exec(function (err, list) {
      if (err) {
        res.json({ msg: "Something went wrong!!!", status: -1, data: err });
      } else {
...
module.exports.getByplaylistId = function (req, res) {
    ...
    SongModel.find({ playlist: id }).populate('playlist').exec(function (err, data) {
        if (err) {
            res.json({ msg: "Something went wrong!!!", status: -1, data: err });
        } else {
